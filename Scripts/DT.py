...
accuracy_dt=metrics.accuracy_score(y_test,DT_pre)*100
Precession=metrics.precision_score(y_test,DT_pre)
ConMat_DT=metrics.confusion_matrix(y_test,DT_pre)
error_rate=1-(accuracy_dt/100)
TP=ConMat_DT[1,1]
TN=ConMat_DT[0,0]
FP=ConMat_DT[0,1]
FN=ConMat_DT[1,0]
...
FPR=FP/float(TN+FP)
TNR=TN/float(TN+FP)
TPR=metrics.recall_score(y_test,DT_pre)
FNR=1-TPR
print("TPR=",TPR)
print("FPR=",FPR)
print("TNR=",TNR)
